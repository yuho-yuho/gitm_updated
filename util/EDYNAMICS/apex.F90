...
!
  implicit none
  real,parameter :: re = 6371.22, eps = 1.e-5
...
!  integer,parameter :: nmax=13
  integer,parameter :: nmax=13 ! am 9/14 titled dipole
...
! am 9/14 test for aligned dipole
    req  = 6378.160,       & ! Equatorial earth radius
!    req  = 6371.22,        & ! Equatorial earth radius
...
!
!obsolete  real,dimension(3),intent(out) :: d1,d2,d3,e1,e2,e3 ! Components of base vectors
...
!  dddthod = 0*cth*sth/d2
  drhodth = alt*cth + (40680925.e0/d)*(cth-sth*dddthod)
  dzetdth =-alt*sth - (40408585.e0/d)*(sth+cth*dddthod)
!  drhodth = alt*cth + (re*re/d)*(cth-sth*dddthod)
!  dzetdth =-alt*sth - (re*re/d)*(sth+cth*dddthod)
...
! Local:
  ...
  if(alt.lt.hr) write(99,'(8(x,e15.8))') hr,alt,clm,sqrror,xlp,x2py2,fz
  if (clm > 1.) then
...
!obsolete  real,dimension(2),intent(out) :: & ! 2-component outputs
!obsolete    f1(2),f2(2)         ! base vectors of Richmond reference
!
! Local:
  integer :: i
...
!obsolete  real,dimension(3),intent(out) :: bhat,d1,d2,d3,e1,e2,e3,b
  real,dimension(3),intent(out) :: bhat,d1,d2,d3,e1,e2,e3,b,f1,f2,f3,g1,g2,g3
!obsolete  real,dimension(2),intent(out) :: f1,f2
